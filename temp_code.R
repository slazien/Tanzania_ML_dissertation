...
if(CLEAN_WORKSPACE) {
      rm(list = setdiff(ls(), c("CLEAN_WORKSPACE", "OS", "USER", "LOAD_WORKSPACE")))
      dev.off()
      gc()
}
...
wssplot <- function(data, nc = 15, seed = 1234, xintercept = 0) {
      ...
      ggplot(tmp_df, aes(x = number_clust, y = wss)) + geom_point() + geom_line() + 
            ...
            geom_vline(xintercept = xintercept, col = "Red", lwd = 1, lty = 3)
}
...
error_cdf_plot <- function(errors_lat, errors_lon, model = "model", save_cdf = FALSE) {
      
      # Calculate euclidean distance from a point to prediction: sqrt(x^2 + y^2)*111 where x is lat/lon error (in km) and y accordingly
      ...
      return(ggplot(km_cdf, aes(x = km, y = prob)) + 
                   ...
                   annotate("text", x = max_km-3, y = 0.85, label = paste0("AUC = ", round(AUC, 2)), size = 8))
}

# OLD AUC FUNCTION WITH MAX_AUC_KM
...
# The above doesn't tell us much, let's see the difference
ggplot(functional_by_year_melt[functional_by_year_melt$variable == "diff",], aes(x = year, y = value)) + 
...
# Now that's better - in past years non-functional dominate, while in recent years there are more functional, so it is seems like the more recently-built wells are indeed more likely to be functional
...
rf.lat <- randomForest(lat_deg ~ groundwater_depth + groundwater_productivity + groundwater_storage + cluster_id + cluster_size + 
                             nearest_city + nearest_dist + is_1992_later + is_50_less, tz[train,], importance = TRUE, replace = FALSE, 
...
rf_cdf <- error_cdf_plot(errors_rf$lat, errors_rf$lon, model = "Random Forest")
print(rf_cdf)
...
gbm.lat <- gbm(lat_deg ~ groundwater_depth + groundwater_productivity + groundwater_storage + cluster_id + cluster_size + 
                     nearest_city + nearest_dist + is_1992_later + is_50_less, data = tz[train,], 
...
ggplot() + 
      ...
      geom_point(data = preds_gbm_df, aes(x = predicted_lon, y = predicted_lat, color = "darkred"), alpha = ALPHA) +
...
gbm_cdf <- error_cdf_plot(errors_gbm$lon, errors_gbm$lat, model = "GBM")
print(gbm_cdf)
...
#                             cluster_size + nearest_city + nearest_dist + is_1992_later + is_50_less, tz)
...
# # Plot predictions against actual locations
# ggplot() + 
#       geom_point(data = preds_las_df, aes(x = actual_lon, y = actual_lat, color = "darkgreen")) + 
#       geom_point(data = preds_las_df, aes(x = predicted_lon, y = predicted_lat, color = "darkred"), alpha = ALPHA) +
#       xlab("Lon") + ylab("Lat") + ggtitle("Actual and predicted locations for LASSO") + theme_minimal() +
#       theme(axis.title.x = element_text(size = 16), 
#             axis.title.y = element_text(size = 16), 
#             plot.title = element_text(face = "bold", size = 20), 
#             legend.title = element_text(face = "bold")) + 
...
xgb_cdf <- error_cdf_plot(errors_xgb$lon, errors_xgb$lat, model = "XGB")
print(xgb_cdf)

# # ------- ANN (nnet) ------- #
# 
# # Enable parallel processing on all cores
# cl <- makeCluster(detectCores())
# registerDoParallel(cl)
# 
# # Perform 10-fold CV and specify custom grid for grid search
# annControl <- trainControl(method = "cv", number = 10)
# annGrid <- expand.grid(.size = c(50, 60, 70), .decay = c(0.01, 0.1, 0.5))
# 
# # Train
# ann.lat <- train(lat_deg ~ groundwater_depth + groundwater_productivity + groundwater_storage + cluster_id + cluster_size + 
#                        nearest_city + nearest_dist + is_1992_later + is_50_less, tz[train,], method = "nnet", linout = TRUE, trace = FALSE,
#                  tuneGrid = annGrid, trControl = annControl, preProc = c("center", "scale"))
# 
# ann.lon <- train(lon_deg ~ groundwater_depth + groundwater_productivity + groundwater_storage + cluster_id + cluster_size + 
#                        nearest_city + nearest_dist + is_1992_later + is_50_less, tz[train,], method = "nnet", linout = TRUE, trace = FALSE, 
#                  tuneGrid = annGrid, trControl = annControl, preProc = c("center", "scale"))
# 
# ann.lat.pred <- predict(ann.lat, tz[-train,])
# ann.lon.pred <- predict(ann.lon, tz[-train,])
#  
# # stopCluster(cl)
# 
# # Metrics
# return_metrics(ann.lat.pred, ann.lon.pred, tz$lat_deg[-train], tz$lon_deg[-train])
# preds_ann_df <- data.frame(actual_lat = tz$lat_deg[-train], actual_lon = tz$lon_deg[-train], predicted_lat = ann.lat.pred, predicted_lon = ann.lon.pred)
# errors_ann <- errors_df(ann.lat.pred, ann.lon.pred, tz$lat_deg[-train], tz$lon_deg[-train])
# 
# # ------- ANN 2 (neuralnet) ------- #
# 
...
#                              nearest_city + nearest_dist + is_1992_later + is_50_less, tz)
# 
...
#                           nearest_city + nearest_dist + is_1992_later + is_50_less, data = tz_mat_scaled[train,], 
#                     hidden = c(50, 50), linear.output = T, stepmax = 1e6, lifesign = "full", lifesign.step = 500, threshold = 0.05)
# 
# 
...
# preds_nn_df <- data.frame(actual_lat = tz$lat_deg[-train], actual_lon = tz$lon_deg[-train], predicted_lat = nn.lat.pred, predicted_lon = nn.lon.pred)
# errors_nn <- errors_df(nn.lat.pred, nn.lon.pred, tz$lat_deg[-train], tz$lon_deg[-train])
# 
# # Plot predictions against actual locations
# ggplot() + 
#       geom_point(data = preds_nn_df, aes(x = actual_lon, y = actual_lat, color = "darkgreen")) + 
#       geom_point(data = preds_nn_df, aes(x = predicted_lon, y = predicted_lat, color = "darkred"), alpha = ALPHA) +
#       xlab("Lon") + ylab("Lat") + ggtitle("Actual and predicted locations for ANN") + theme_minimal() +
#       theme(axis.title.x = element_text(size = 16), 
#             axis.title.y = element_text(size = 16), 
#             plot.title = element_text(face = "bold", size = 20), 
#             legend.title = element_text(face = "bold")) + 
#       scale_color_manual(name = "Location type", values = c("darkgreen", "darkred"), labels = c("Actual location", "Predicted location"))
# 
# nn_cdf <- error_cdf_plot(errors_nn$lon, errors_nn$lat, model = "ANN", AUC_max_km = AUC_MAX_KM)
# print(nn_cdf)
...
avg_cdf <- error_cdf_plot(errors_avg$lat, errors_avg$lat, model = "Weighted average")
print(avg_cdf)
...
# Aggregate plot of CDF
grid.arrange(rf_cdf, gbm_cdf, xgb_cdf, avg_cdf, nrow = 2, ncol = 2)
...
xgb.lat.imp.plot <- ggplot(xgb_importance_lat, aes(x = reorder(Feature, Gain), y = Gain)) + 
      ...
      theme(axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16), 
            plot.title = element_text(face = "bold", size = 20), legend.title = element_text(face = "bold")) + coord_flip()
...
# RF VarImp
rf.lat.imp <- as.data.frame(rf.lat$importance)
rf.lat.imp$variable <- rownames(rf.lat.imp)
rownames(rf.lat.imp) <- 1:nrow(rf.lat.imp)
rf.lon.imp <- as.data.frame(rf.lon$importance)
rf.lon.imp$variable <- rownames(rf.lon.imp)
rownames(rf.lon.imp) <- 1:nrow(rf.lon.imp)

rf.lat.imp.plot <- ggplot(rf.lat.imp, aes(x = reorder(variable, IncNodePurity), y = IncNodePurity)) + 
...
rf.lon.imp.plot <- ggplot(rf.lon.imp, aes(x = reorder(variable, IncNodePurity), y = IncNodePurity)) + 
      geom_bar(stat = "identity", fill = "steelblue4") + xlab("Variable") + ylab("Increase in node purity") + 
      ggtitle("RF variable importance - lon") + theme_minimal() + 
      theme(axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16), 
            plot.title = element_text(face = "bold", size = 20), legend.title = element_text(face = "bold")) + coord_flip()

grid.arrange(rf.lat.imp.plot, rf.lon.imp.plot)